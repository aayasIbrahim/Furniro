  ...
  return (
    <div className={`group relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 ${product.isFeatured ? "border-2 border-purple-600" : "border border-gray-100"}`}>
      ...
      <div className="relative h-[360px] overflow-hidden">
        ...
        {/* Badge */}
        {product.badge && (
          <span className={`absolute top-4 left-4 text-white text-xs font-semibold px-3 py-1 rounded-full ${product.badge === "New" ? "bg-green-500" : "bg-red-500"}`}>
            {product.badge}
          </span>
        )}
      ...
      <Link href={`/products/${product.id}`}>
        <div className="p-6 text-center">
          <h3 className="font-poppins text-xl font-semibold text-gray-800 mb-1">{product.name}</h3>
          <p className="text-gray-500 text-sm mb-3">{product.description}</p>
          <div className="flex justify-center items-center space-x-2">
            <span className="text-lg font-bold text-gray-800">Rp {product.price.toLocaleString()}</span>
            {product.oldPrice && (
              <span className="text-gray-400 text-sm line-through">Rp {product.oldPrice.toLocaleString()}</span>
            )}
          </div>
        </div>
      </Link>
